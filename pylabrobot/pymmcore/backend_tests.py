...
from pylabrobot.pymmcore.backend import (
  USE_PYMMCORE,
  USE_USEQ,
  PymmcoreImagerBackend,
  StagePlateCalibration,
)
...
@unittest.skipUnless(USE_USEQ and USE_PYMMCORE, "requires useq-schema and pymmcore-plus")
class TestPymmcoreImagerBackend(unittest.IsolatedAsyncioTestCase):
  async def asyncSetUp(self):
    self.core = _FakeCore()
    self.plate = cor_96_wellplate_360uL_Fb(name="plate")
    self.backend = PymmcoreImagerBackend(
      core=cast("CMMCorePlus", self.core),
  ...
  async def test_unknown_channel_preset_raises_on_setup(self):
    backend = PymmcoreImagerBackend(
      core=cast("CMMCorePlus", _FakeCore()), channel_map={ImagingMode.GFP: "nonexistent"}
    )
