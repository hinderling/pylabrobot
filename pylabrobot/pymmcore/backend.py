...
class PymmcoreImagerBackend(ImagerBackend):
  ...
  core's MDA runner, so any Micro-Manager-supported microscope can be used through
  PLR's standard :class:`~pylabrobot.legacy.plate_reading.imager.Imager` API.
  ...
  async def _run_sequence(
    ...
    """Run a useq MDASequence on the core's MDA runner, returning
  ...
  async def capture(
    ...
    **backend_kwargs: Any,
    ...
    if coverage is not None and coverage != (1, 1):
      rows, cols = coverage
      grid_plan = useq.GridRowsColumns(rows=rows, columns=cols, overlap=(overlap, overlap))
    ...
    frames = await self._run_sequence(sequence)
